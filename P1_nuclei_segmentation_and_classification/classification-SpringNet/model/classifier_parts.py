...
class Spring(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mid_channels=None, dropout=0.1, skip=False):
        super().__init__()
        self.skip = skip
        if not mid_channels:
            mid_channels = out_channels
        self.spring = nn.Sequential(
            nn.Conv2d(in_channels, mid_channels, kernel_size=(3, 3), padding=1, stride=(1, 1), bias=False, dtype=torch.float32),
